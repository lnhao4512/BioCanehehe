...
const Register = () => {
  ...
  return (
    <div className="min-h-screen bg-company-offWhite flex items-center justify-center px-4 py-12">
      <div className="bg-white p-10 rounded-3xl shadow-xl w-full max-w-md relative overflow-hidden">
        ...
        <h2 className="text-3xl font-bold text-center text-company-dark mb-2">Tạo tài khoản mới</h2>
        <p className="text-center text-company-dark/60 mb-8">Gia nhập cộng đồng BioCane</p>
        
        {error && <div className="bg-company-red/10 text-company-red p-3 rounded-lg mb-6 text-sm text-center">{error}</div>}

        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-company-dark mb-2">Họ và tên</label>
            <div className="relative">
              <User className="absolute left-3 top-1/2 -translate-y-1/2 text-company-dark/40" size={20} />
              <input
                type="text"
                name="name"
                required
          ...
          <div>
            <label className="block text-sm font-medium text-company-dark mb-2">Mật khẩu</label>
            <div className="relative">
        ...
        <p className="text-center mt-8 text-company-dark/70">
          Đã có tài khoản? <Link to="/login" className="text-company-green font-semibold hover:underline">Đăng nhập ngay</Link>
        </p>
      </div>
    </div>
  );
