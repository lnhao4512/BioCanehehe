...
const Factory3DModel = () => {
  ...
  const toggleFullscreen = () => {
    if (!document.fullscreenElement) {
      // Optimistically update state for smoother UI transition before the browser native action
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      // Keep state in sync in case user presses ESC to exit
      if (!document.fullscreenElement) {
...
// ZoomHandler removed because zooming is disabled in normal view
  ...
  return (
    <div 
      ref={containerRef} 
      className={`w-full h-full relative group z-10 pointer-events-auto flex flex-col transition-colors duration-700 ease-in-out ${isFullscreen ? 'bg-[#f8f5f0]' : ''}`}
    >
      ...
      <div className={`absolute left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 md:gap-3 z-40 pointer-events-auto transition-all duration-700 ease-out w-full px-2 max-w-[600px] ${isFullscreen ? 'bottom-6 md:bottom-10 scale-100' : 'bottom-0 md:-bottom-4 lg:-bottom-8 scale-95 md:scale-100'}`}>
        <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-md px-4 md:px-8 py-2 md:py-3 rounded-2xl md:rounded-full shadow-[0_8px_30px_rgba(0,0,0,0.06)] text-xs md:text-sm font-medium text-company-dark/70 dark:text-gray-300 flex flex-wrap items-center justify-center gap-2 md:gap-3 border border-company-green/10 dark:border-gray-800 w-full text-center transition-all duration-500 hover:shadow-[0_8px_30px_rgba(0,0,0,0.1)]">
          ...
          {isFullscreen && (
            <>
              <span className="mx-1 md:mx-2 text-company-green/30 dark:text-gray-600 opacity-0 animate-[fadeIn_0.5s_ease-out_forwards]">•</span>
              <span className="opacity-0 animate-[fadeIn_0.5s_ease-out_0.1s_forwards]">{t('scrollZoom')}</span>
              <span className="mx-1 md:mx-2 text-company-green/30 dark:text-gray-600 opacity-0 animate-[fadeIn_0.5s_ease-out_0.2s_forwards]">•</span>
              <span className="opacity-0 animate-[fadeIn_0.5s_ease-out_0.3s_forwards]">{t('rightPan')}</span>
            </>
          )}
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-3 w-full">
          <button 
            ...
            className="flex-1 min-w-[140px] sm:flex-none justify-center bg-white/95 dark:bg-gray-900/95 backdrop-blur-md hover:bg-company-offWhite dark:hover:bg-gray-800 px-4 py-2 md:px-6 md:py-3 rounded-xl md:rounded-full shadow-[0_4px_15px_rgba(0,0,0,0.04)] hover:shadow-[0_6px_20px_rgba(40,167,69,0.15)] text-[11px] md:text-sm font-semibold text-company-dark dark:text-white flex items-center gap-2 border border-black/5 dark:border-gray-800 hover:border-company-green/30 hover:text-company-green transition-all duration-500 hover:-translate-y-0.5 active:translate-y-0"
          >
            {autoRotate ? <Pause size={16} className="shrink-0" /> : <Play size={16} className="shrink-0" />}
            <span className="whitespace-normal text-center leading-tight">{autoRotate ? t('stopRotate') : t('startRotate')}</span>
          </button>
          <button 
          ...
          >
            ...
            <span className="whitespace-normal text-center leading-tight">{isFullscreen ? t('exitFullscreen') : t('fullscreen')}</span>
          </button>
          <button 
            onClick={handleReset}
            className="flex-1 min-w-[140px] sm:flex-none justify-center bg-white/95 dark:bg-gray-900/95 backdrop-blur-md hover:bg-company-offWhite dark:hover:bg-gray-800 px-4 py-2 md:px-6 md:py-3 rounded-xl md:rounded-full shadow-[0_4px_15px_rgba(0,0,0,0.04)] hover:shadow-[0_6px_20px_rgba(40,167,69,0.15)] text-[11px] md:text-sm font-semibold text-company-dark dark:text-white flex items-center gap-2 border border-black/5 dark:border-gray-800 hover:border-company-green/30 hover:text-company-green transition-all duration-500 hover:-translate-y-0.5 active:translate-y-0"
          >
            ...
            <span className="whitespace-normal text-center leading-tight">{t('resetView')}</span>
          </button>
          <button 
            onClick={() => setShowHotspots(!showHotspots)}
            className="flex-1 min-w-[140px] sm:flex-none justify-center bg-company-darkGreen dark:bg-company-lightGreen text-white px-4 py-2 md:px-6 md:py-3 rounded-xl md:rounded-full shadow-[0_4px_15px_rgba(0,0,0,0.04)] hover:shadow-[0_6px_20px_rgba(40,167,69,0.25)] text-[11px] md:text-sm font-semibold flex items-center gap-2 border border-transparent hover:bg-company-green transition-all duration-500 hover:-translate-y-0.5 active:translate-y-0"
          >
            <Info size={16} className="shrink-0" />
            <span className="whitespace-normal text-center leading-tight">{showHotspots ? t('hideHotspots') : t('showHotspots')}</span>
          </button>
        </div>
      </div>
    </div>
